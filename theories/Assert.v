...
Definition setoid (p: trace -> Type) :=
forall tr0, p tr0 -> forall tr1, bisim tr0 tr1 -> p tr1.  
Definition assertS := state -> Type.
Definition assertT := { p:  trace -> Type & setoid p}.

Definition assertS_and (u1 u2: assertS) := 
fun st =>  prod (u1 st) (u2 st).

Infix "andS" := assertS_and (at level 60, right associativity).

Definition assertT_and0 (p1 p2: assertT) :=
let: existT f0 h0 := p1 in
let: existT f1 h1 := p2 in
fun tr => prod (f0 tr) (f1 tr).  
...
destruct p1 as [f0 h0]. destruct p2 as [f1 h1]. exists (fun tr => prod (f0 tr) (f1 tr)). 
move => tr0 [h2 h3] tr1 h4. split. 
* have := h0 _ h2 _ h4; apply. 
* have := h1 _ h3 _ h4; apply.
Defined.   
...
Proof. move => u st0 h0. done. Qed. 

Definition satisfy (p:assertT) (tr: trace): Type := 
let: existT f0 h0 := p in f0 tr.
...
Inductive ttS: state -> Type := | ttS_intro: forall st, ttS st.  
Definition ffS: assertS := fun st => Empty_set.

Inductive ttt: trace -> Type := | ttt_intro: forall tr, ttt tr.
...
Lemma satisfy_monotone: forall p0 p1, 
...
have := satisfy_monotone h1. apply. 
have := satisfy_monotone h0 h2. by apply. 
Qed.  
...
Proof. move => u1 u2 st h1; by inversion h1. Qed.   
Lemma andS_monotone: forall u1 u1' u2 u2',
u1 ->> u1' -> u2 ->> u2' -> (u1 andS u2) ->> (u1' andS u2').
Proof. 
...
Definition singleton (u:assertS): trace -> Type :=
fun tr => { st: state & prod (u st) (bisim tr (Tnil st))}.

Lemma singleton_setoid: forall u, setoid (singleton u). 
Proof. 
move => u. move => tr0 h0 tr1 h1. move: h0 => [st0 [h0 h2]]. 
foo h2. foo h1. exists st0. split; [apply h0 | apply bisim_reflexive]. 
Qed. 
...
Lemma mk_singleton_nil: forall u st,
...
Definition dup (u: assertS): trace -> Type :=
fun tr => { st: state & prod (u st) (bisim tr (Tcons st (Tnil st))) }. 

Lemma dup_monotone: forall (u0 u1: assertS),
u0 ->> u1 ->
forall tr, dup u0 tr -> dup u1 tr. 
Proof. 
move => u0 u1 hu tr [st0 [h0 h1]]. foo h1. foo H1. 
exists st0. split. have := hu _ h0; apply. apply bisim_reflexive.  
Qed. 
...
Proof. move => u v h0 tr0. simpl. apply dup_monotone => //. 
Qed.  

CoInductive follows (p: trace -> Type): trace -> trace -> Type :=
...
Lemma follows_setoid: forall (p: trace -> Type)
...
- clear COINDHYP. foo h1. have h0 := bisim_hd h2. symmetry in h0. 
  have := follows_nil h0 (hp _ X _ h2). apply. 
- foo h2. foo h1. have := follows_delay st (COINDHYP _ _ X _ H3 _ H2); apply. 
Qed.
...
- foo h1. apply follows_nil. reflexivity. apply X. 
- foo h1. have := follows_delay st (COINDHYP _ _ X _ H2). apply. 
Qed. 

Lemma follows_setoid_R: forall (p: trace -> Type)
(hp: forall tr0, p tr0 -> forall tr1, bisim tr0 tr1 -> p tr1), 
...
- apply follows_nil. symmetry. have := bisim_hd h1. apply. 
  have := hp _ X _ h1; apply.  
- foo h1. have := follows_delay st (COINDHYP _ _  X _ H2). apply.
Qed.

Lemma follows_monotone: forall p q,
(forall tr, p tr -> q tr) -> 
forall tr0 tr1, follows p tr0 tr1 -> follows q tr0 tr1.
Proof. 
move => p q hpq. cofix hcoind. move => tr0 tr1 h0. foo h0. 
- apply follows_nil => //. have := hpq _ X; by apply.
- have := follows_delay _ (hcoind _ _ X). by apply. 
Qed.    
...
- simpl in X. move: X => [st0 [h0 h1]]. foo h1. simpl; apply bisim_reflexive.  
- have := bisim_cons st (COINDHYP _ _ X); apply.
Qed.
...
Proof. 
move => u v. cofix hcoind. move => tr h0 h1. inversion h0. 
- apply follows_nil => //. exists st. split; last by apply bisim_reflexive. 
  clear H H1. subst. inversion h1. clear H0 H1. subst. 
  split; [ have := singleton_nil X; apply | have := singleton_nil X0; apply]. 
- subst. foo h0. foo h1. have := follows_delay st (hcoind _ X0 X1); by apply. 
Qed.  
...
- clear h0 H H0 H1. rewrite [lastdup _]trace_destr. simpl. 
  move: X => [st1 [h0 h1]]. foo h1. apply follows_nil => //. exists st1.
  split; [done | apply bisim_reflexive]. 
- clear h0 H H1. rewrite [lastdup _]trace_destr. simpl. 
  have := follows_delay st (COINDHYP _ X). apply. 
Qed.

Definition append (p1 p2: trace -> Type): trace -> Type :=
fun tr => { tr': trace &  prod (p1 tr') (follows p2 tr' tr) }.

Infix "*+*" := append (at level 60, right associativity).

Lemma append_monotone: forall (p0 p1 q0 q1: trace -> Type),
...
- apply follows_nil => //. have := hq _ X; apply. 
- have := follows_delay st (COINDHYP _ _ X); apply. 
Qed.    

Lemma append_monotone_L: forall (p0 p1 q: trace -> Type),
(forall tr, p0 tr -> p1 tr) ->
...
Lemma append_monotone_R: forall (p q0 q1: trace -> Type),
(forall tr, q0 tr -> q1 tr) ->
forall tr, (append p q0 tr) -> (append p q1 tr).
Proof. 
move => p q0 q1 hq. have := (@append_monotone p p _ _ _ hq). apply => //.    
Qed.  

Lemma append_setoid: forall (p0 p1: trace -> Type),
...
move => h2. have := append_monotone _ _ h2. apply. apply h0. apply h1. 
Qed.   
...
have := append_monotone_L h0.  apply.
Qed.   
...
have := (@append_monotone q q p1 p2). apply. done. apply h0. 
exact h1.
Qed.    
...
foo h1. destruct X as [st0 [h1 h2]]. foo h2. foo H1. simpl in h0. exists st0. 
...
foo h1. foo X. destruct X0 as [st0 [hv h0]]. foo h0. simpl in hu. simpl. 
exists st0. by split; last apply bisim_reflexive. 
Qed.       
...
foo h1. destruct X as [st0 [hv h0]]. foo h0. simpl in hu. exists st0. 
...
CoInductive iter (p: trace -> Type): trace -> Type :=
...
* cofix COINDHYP1. move => tr. move => tr0 h0 tr1 h1. foo h0. 
  - apply follows_nil. symmetry. have := bisim_hd h1. apply. 
    have := COINDHYP _ X _ h1. apply.  
  - foo h1. have := follows_delay st (COINDHYP1 _ _ X _ H2). apply.
* move => tr0 h1 tr1 h2. foo h1. 
  - foo h2. apply iter_stop. 
  - have := iter_loop X (h0 _ _ X0 _ h2). apply. 
Qed. 

Lemma iter_cont: forall (p0 p1: trace -> Type),
(forall tr, p0 tr -> p1 tr) ->
...
* cofix COINDHYP0. move => tr0 tr1 h0. foo h0. 
  - apply follows_nil. reflexivity. have := COINDHYP _ X; apply. 
  - have := follows_delay st (COINDHYP0 _ _ X). apply. 
* move => tr0 h0. foo h0. 
  - apply iter_stop. 
  - have := iter_loop (hp _ X)  (h _ _ X0). apply. 
Qed.  


Lemma iter_append_dup: forall u p tr,
...
- move: X => [tr1 [h2 h1]]. clear h2 h0. (*
  have h2 := follows_hd X0.  rewrite -h2 in h0 => {h2}.   *)
  move: tr tr1 tr0 h1 X0. cofix hcoind1. 
  move => tr0 tr1 tr2 h0 h1. foo h0.
  - move: X => [st0 [h0 h3]]. foo h3. foo H1. 
    foo h1. foo X. have := follows_delay _ (hcoind _ h0 X0). by apply.  
  - foo h1. have := follows_delay _ (hcoind1 _ _ _ X X0). by apply. 
Qed. 
...
Definition updt (u: assertS) (x:id) (a: expr): trace -> Type := 
fun tr => { st: state & prod (u st) (bisim tr (Tcons st (Tnil (update x (a st) st))))}.
...
exists (fun tr => { x: A & satisfy (p x) tr}).
...
- destruct X as [st0 [h1 h2]]. inversion h1. 
- apply infinite_delay. apply (h0 _ _ X). 
Qed. 



(* extensions *)
Lemma iter_split_1: forall p tr, iter p tr -> sum (singleton ttS tr) (append p (iter p) tr).
Proof. 
move => p tr h0. foo h0. 
...
Lemma iter_split_2: forall tr p, sum (singleton ttS tr) (append p (iter p) tr) -> iter p tr. 
Proof. 
move => tr p h. foo h. 
- move: X => [st [h0 h1]]. foo h1.  apply iter_stop.
- move: X => [tr0 [h0 h1]]. have := iter_loop h0 h1. by apply. 
Qed.      
...
- foo h1. have := iter_loop X. apply. clear X. move: tr1. cofix hcoind0. 
  case. move => st. apply follows_nil => //. apply iter_stop. 
  move => st0 tr0. have := follows_delay _ (hcoind0 _). by apply. 
- have := iter_loop X. apply. clear X. move: tr tr0 tr1 X0 h1.
  cofix hcoind0. move => tr0 tr1 tr2 h0 h1. foo h0.
  - have h0 := follows_hd h1. apply follows_nil => //. 
    have := hcoind _ _ X h1. by apply. 
  - foo h1. have := follows_delay _ (hcoind0 _ _ _ X X0). by apply. 
Qed.            
...
- destruct X as [st0 [_ h0]]. foo h0. by apply iter_stop. 
- have := iter_unfold_1 X. by apply. 
Qed. 
...
move => tr0 tr1 h0. foo h0. 
- move => h0. foo h0. done. 
- move => h0. have := iter_loop X. apply. clear X. move: tr tr0 tr1 X0 h0. 
  cofix hcoind2. move => tr0 tr1 tr2 h0. foo h0. 
  - move => h0. have h1 := follows_hd h0. apply follows_nil. done. 
    have := hcoind _ _ X h0. done. 
  - move => h0. foo h0. apply follows_delay.  have := hcoind2 _ _ _ X X0. 
    done. 
Qed.                   
...
Inductive fin: trace -> state -> Type :=
| fin_nil: forall st, fin (Tnil st) st
| fin_delay: forall st st' tr, fin tr st' -> fin (Tcons st tr) st'.



Lemma append_fin: forall p q tr0 tr1, p tr0 -> q tr1 -> fin tr0 (hd tr1) ->
(p *+* q) (tr0 +++ tr1). 
...
Definition last (p: trace -> Type): assertS := 
fun st => {tr: trace & prod (p tr) (fin tr st)}.

Lemma last_cont: forall (p q: trace -> Type),
(forall tr, p tr -> q tr) ->
last p ->> last q.
Proof. 
...
foo h1. simpl. exists tr0. split => //. move: tr0 h0 X. cofix hcoind. 
move => tr0 h0 h1. foo h1. 
- apply follows_nil => //. simpl in h0. exists st. split => //.
  apply bisim_nil. 
- clear h0. move: X => [tr1 [_ h1]]. move: tr1 tr tr0 h1 X0. cofix hcoind0.
  move => tr0 tr1 tr2 h0 h1. foo h0. 
  - move: X => [st0 [h0 h2]]. foo h2. foo H1. foo h1. foo X. 
    have := follows_delay _ (hcoind _ h0 X0). by apply.   
  - foo h1. have := follows_delay _ (hcoind0 _ _ _ X X0). apply.
Qed. 
...
Proof. 
move => v. move => tr0 [tr1 [h0 h1]]. move: h0 => [st0 [h0 h2]]. foo h2.
foo H1. foo h1. foo X. simpl. exists (Tcons (hd tr') tr'). split => //.
apply follows_delay => //. move: tr' h0 X0. cofix hcoind. 
move => tr0 h0 h1. foo h1. 
- apply follows_nil => //. simpl in h0. exists st. split => //.
  apply bisim_nil. 
- clear h0. move: X => [tr1 [_ h1]]. move: tr1 tr tr0 h1 X0. cofix hcoind0.
...
* exists tr2. split => //. clear h1 h0.
  ...
  - move => st1 tr0 h0. foo h0. rewrite trace_append_cons. 
    apply follows_delay. have := hcoind _ H2. by apply. 
* clear X h0. move h0: (hd tr1) => st1. rewrite h0 in h2. 
