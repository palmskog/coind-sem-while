...
have: forall st, fin tr0 st -> {tr1: trace &  {h : q tr1 & hd tr1 = st}}.
* move => st0 h2. have h3: singleton (last p) (Tnil st0).
  exists st0. split; last apply bisim_reflexive. exists tr0. split => //. 
  have := h0 _ h3 => {h0 h3}. move => [tr1 h0]. foo h0. exists tr1. done. 
move => h2. clear h0 h1. exists (f h2). move: tr0 h2. cofix hcoind. 
move => t0. dependent inversion t0.
- move => h0. rewrite [f _]trace_destr. simpl. destruct (h0 s (fin_nil s)).
  destruct s0. destruct x. by apply follows_nil => //. simpl in e. 
  rewrite e. apply follows_nil => //. rewrite -e. done.
...
- move => m w r hafter. 
  ...
  * have h: after ([|w|] *** p1 *** [|v|]) p2. 
    * have := after_monotone_L (@Append_assoc_R _ _ _). apply. 
      ...
      foo h1. have := semax_post X2 h0 => {h0}. move =>[tr0 [h0 h1]].
  ...
  have hs1 := IHX1 m w (p2 *** r) hr => {IHX1 hr}. 
  ...
  * destruct p1 as [p1 hp1]. destruct p2 as [p2 hp2]. destruct r as [r hr]. 
    ...
    * apply append_assoc_L. have := append_assoc_R h0 => {h0}. move => h0.
      have := append_fin _ X. apply => //. have := append_singleton _ h0. apply => //. 
      apply append_setoid => //. clear h0 X. move => h0. 
      have := hafter _ h0 => {hafter h0}. move => [tr2 h0].
      move h2: (hd tr0) => st0. rewrite h2 in h1. 
      exists (cut h1 tr2). move: tr1 st0 h1 tr0 h2 tr2 h0. move => tr1 st0 h0.
      dependent induction h0. 
      - move => tr0 h0 tr2 h1. rewrite trace_append_nil in h1. simpl. done. 
      - move => tr0 h2 tr2 h1. simpl. rewrite trace_append_cons in h1. foo h1. 
        have := IHh0  _ _ _ X. apply => //. 
  have hs2 := IHX2 m (Last (([|w|]) *** p1 *** ([|v|]))) r hr => {IHX2 hr}. 
  have: (Last (([|w|]) *** p1 *** ([|v|]))) ->> v. 
...
- move => m w r hafter. 
  * have haftertt: after ([|w andS u|] *** p) r. 
    * destruct p as [p hp]. destruct r as [r hr]. simpl. clear IHX1 IHX2.
    ...
    have := IHX1 m _ _ haftertt => {IHX1 haftertt}. move => hsemax.
    ...
    * move => st0 [[h0 [h1 h3]] h2]. split => //. split => //. 
      ...
      move => h5. foo h5. have := len_S H1. done. 
     * move => st0 [h0 h1]. split => //. clear h1. destruct p as [p hp]. 
       ...
       by apply fin_delay. 
    have := IHX2 m _ _ haftertt => {IHX2 haftertt}. move => hsemax.
    have := tsemax2_conseq _ _ hsemax. apply => {hsemax}.
    * move => st0 [[h0 [h1 h3]] h2]. split => //. split => //. 
      have := Len_monotone (@Append_assoc_R _ _ _) h3. clear h3. move => h3. 
      destruct p as [p hp]. destruct r as [r hr]. clear h1 h2. move => tr0 [h1 h2].
...
- move => m w r hafter. set J0 := Last (<< u0 andS w >> *** (Iter (p *** << u >>))). 
  ...
  * destruct p as [p hp]. destruct r as [r hr]. simpl. move => tr0 h4 h5. 
     ...
     - foo h6. destruct X0 as [tr1 [h0 h4]]. destruct h0 as [st1 [h0 h5]]. 
       foo h5. foo h4. rewrite /eval_true in h1. rewrite /eval_false in h0. 
       rewrite h1 in h0. foo h0. 
     - destruct X0 as [tr2 [h0 h4]]. destruct tr2.
       - clear h0. foo h4. destruct X0 as [st0 [_ h0]]. foo h0. foo H1. 
         foo X1. foo h6. foo h3. 
       - foo h4. foo X1. foo h6. foo h3.  

    have hsemax := semax_while (@assertS_imp_refl _) X => {X}.
    have h0 := semax_post hsemax => {hsemax}.  
    have hu : u st0. have := tsemax2_complete_aux3 a0 h2. done. 
   ...
   * have := after_monotone_L _ hafter => {hafter}. apply. 
    ...
    destruct h2 as [st1 [h2 h7]]. foo h7. foo H1. destruct h2 as [h2 h0]. 
    foo h6. foo h5. foo X. destruct h4 as [tr1 [h4 h5]]. destruct h4 as [st0 [h4 h6]]. 
    foo h6. foo H1. foo h5. simpl in H2. simpl in h0. simpl in h2. simpl in X0.
    simpl. simpl in h1. foo X. 
    have : append (iter (append p (dup u))) 
    (append (iter (append p (dup u))) (singleton (eval_false a))) (tr' +++ tr'0).
    * exists tr'. split => //. clear hu hfalse X0 h2 h0 h4 h1 h3. move: tr' tr'0 H2 X1.
      ...
      apply follows_delay. have := hcoind _ _ H h0. done.
    move => h5.
    ...
    * rewrite /P. simpl. exists (Tnil (hd tr')). split. exists (hd tr'). 
      ...
      apply follows_nil => //. have := hd_append H2 (refl_equal _). by apply. move => hP. 
    ...
    have := tsemax2_complete_aux hfinite (refl_equal _) h3 X1 => {X1}.
  ...
  * move => n st0 [h0 [h1 h2]]. split => //. split => //. 
  ...
  apply Dup_monotone. move => st1 [h3 h4]. have := a0 _ h3. done. 
  ...
  * clear hn0 hpre. move => n hn st0 h0. rewrite /J0 in h0. 
    ...
    * destruct h0 as [h0 h1]. split => //. destruct p as [p hp]. 
      ...
      apply h1. simpl. split => //. have hu := tsemax2_complete_aux3 a0 h0. 
  ...
  * move => n. have hn : (S n) > 0; first by lia.
    ...
    * move => st0 [h0 [h1 [h2 h3]]]. split => //. split => //. split => //. 
    ...
    have := tsemax2_conseq _ _ (IHX (S n) J0 Q _) => {IHX}. apply => //.  
...
(* prove after ([J0] *** p *** [u]) Q *)
    ...
    * move => tr0. simpl. move => [st0 [h0 h1]]. foo h1. foo H1. destruct h0 as [h0 h1]. 
   ...
   * clear h0. have := append_assoc_L h3. move => h0. 
     ...
     have := append_assoc_R h0 => {h0}. move => h0.    
     have := append_assoc_R h0 => {h0}. move => h0.    
     have := append_monotone_L _ h0 => {h0}. apply. clear tr2. 
     ...
     * clear tr0 h0. move => tr0 h0. have := append_assoc_R h0 => {h0}.
       ...
       have := append_monotone_L _ h0. apply. apply append_singleton => //.
   ...
   have := follows_monotone (append_monotone_L (@append_assoc_R _ _ _)) h0 => {h0}. done. 
  ...
  * have h0: (u0 andS w) ->> J0.
     ...
     * destruct p as [p hp]. destruct r as [r hr]. move => st0. move => [h1 h2].
        ...
        * apply h2. simpl. split => //. destruct h3 as [tr1 [h3 h5]].
           ...
           move => tr0 tr1 h0. foo h0. destruct X0 as [tr0 [h0 h1]]. 
           ...
           cofix hcoind. move => tr0 tr1 h0. foo h0. destruct X0 as [tr0 [h0 h1]]. 
           destruct h0 as [st1 [h0 h2]]. foo h2. foo h1. apply follows_nil => //.
           apply follows_delay. have := hcoind _ _ X0. done. 
        foo h5. have := len_S H1. done. 
  ...
  * move => st0. rewrite /J0. rewrite /J1. move => [h0 [h1 h2]]. split. 
    ...
    * by split => //.
    ...
    foo h1. foo H1. destruct h0 as [h0 h1]. simpl. exists (Tnil st1). 
...
- move => m w r hafter. have h: after ([|w|] *** p2) r. 
  ...
  have := IHX m _ _ h => {hafter h IHX}. move => htsemax. 
  have := tsemax2_conseq _ _ htsemax => {htsemax}. apply. 
  * move => st0 [h0 [h1 h2]]. split => //. have := a _ h0. done. 
...
- move => m w r hafter. 
  ...
  * move => x. apply X. move hp: (p x) => q. destruct q as [q hq]. 
    ...
    exists x. rewrite hp. simpl. done. apply follows_delay. have := hcoind _ _ X0. done.
...
(*
- move => m w r. have hs1 := IHX1 m w (p2 *** r) => {IHX1}. 
  have: (Len ((p1 *** p2) *** r) m) ->> Len ((p1 *** [|v|]) *** p2 *** r) m.
  * apply Len_monotone. have := impT_conseq_L (@Append_monotone_L _ p1 _ _). apply.
    apply Append_Singleton. by apply Append_assoc_R. 
  move => h0. have : tsemax2 (u andS w andS Len ((p1 *** p2) *** r) m)
  ...
  have := tsemax2_conseq_L _ hs1 => {hs1}. apply. move => st0 [h1 [h2 h3]].
  split => //. split => //. have := h0 _ h3. done. clear h0 hs1. move => hs1.
  have hs2 := IHX2 m (Last (([|w|]) *** p1 *** ([|v|]))) r => {IHX2}. 
  have: (Last (([|w|]) *** p1 *** ([|v|]))) ->> v. 
  * move => st0 h0. have := Last_monotone (@Append_assoc_R _ _ _) h0.
    clear h0. move => h0. have := Last_chop_sglt h0. done. 
  move => h0. 
  ...
  * have := tsemax2_conseq_L _ hs2 => {hs2}. apply. move => st0 [h1 h2].
  ...
  apply Append_monotone_R. apply Append_Singleton. 
- move => m w r. 
  have hpre: (Last (([|u andS w|]) *** p) andS Len r m) ->>
  Last (([|w|]) *** (<< u >>) *** p) andS Len r m.
  * move => st0 [h0 h1]. split => //. clear h1. destruct p as [p hp]. simpl in h0. 
    destruct h0 as [tr0 [h0 h1]]. simpl. exists (Tcons (hd tr0) tr0). 
    destruct h0 as [tr1 [h0 h2]]. destruct h0 as [st1 [h0 h4]]. 
    foo h4. foo h2. destruct h0 as [h0 h2]. split. exists (Tnil (hd tr0)). 
    split. exists (hd tr0); split => //. apply bisim_reflexive. 
    apply follows_nil => //. exists (Tcons (hd tr0) (Tnil (hd tr0))). 
    split. exists (hd tr0). split => //. apply bisim_reflexive. 
    apply follows_delay. apply follows_nil => //. apply fin_delay => //. 
  apply tsemax2_ifthenelse. 
  * have := IHX1 m (u andS w) r. clear IHX1. move => hs1. 
    have : tsemax2 ((u andS eval_true a) andS (u andS w) andS Len (p *** r) m) s1
    (Last (([|w|]) *** (<< u >>) *** p) andS Len r m). 
    * have := tsemax2_conseq_R hpre hs1 => {hs1}. apply.
    clear hs1. move => hs1. 
    have := tsemax2_conseq_L _ hs1 => {hs1}. apply.
    * move => st0 [[h0 [h1 h3]] h2]. split => //. split => //.
      move => tr0 [h4 h5]. have : len m (Tcons (hd tr0) tr0).
      apply h3 => {h3}. split => //. destruct p as [p hp]. 
      destruct r as [r hr]. simpl. simpl in h5. destruct h5 as [tr1 [h5 h6]].
      have := follows_hd h6 => h7. 
      exists (Tcons (hd tr1) tr1). split. exists (Tcons (hd tr1) (Tnil (hd tr1))). 
      split. exists (hd tr1). split. rewrite h7. rewrite h4. done.
      apply bisim_reflexive. apply follows_delay. apply follows_nil => //. 
      rewrite h7. apply follows_delay. done. clear h0 h1 h2 h3 h4 h5 st0. 
      move => h0. foo h0. have := len_S H1. done. 
  * have := IHX2 m (u andS w) r. clear IHX2. move => hs2. 
    have : tsemax2 ((u andS eval_false a) andS (u andS w) andS Len (p *** r) m) s2
    (Last (([|w|]) *** (<< u >>) *** p) andS Len r m). 
    * have := tsemax2_conseq_R hpre hs2 => {hs2}. apply.
    clear hs2. move => hs2. 
    have := tsemax2_conseq_L _ hs2 => {hs2}. apply.
    * move => st0 [[h0 [h1 h3]] h2]. split => //. split => //.
      move => tr0 [h4 h5]. have : len m (Tcons (hd tr0) tr0).
      apply h3 => {h3}. split => //. destruct p as [p hp]. 
      destruct r as [r hr]. simpl. simpl in h5. destruct h5 as [tr1 [h5 h6]].
      ...
      rewrite h7. apply follows_delay. done. clear h0 h1 h2 h3 h4 h5 st0. 
      move => h0. foo h0. have := len_S H1. done.
- move => m w r. set J0 := Last (<< u0 andS w >> *** (Iter (p *** << u >>))). 
  set J1 := (Iter (p *** << u >>) *** [|eval_false a|] *** r). 
  set Q := <<u>> *** (Iter (p *** <<u>>)) *** [|eval_false a|] *** r.
...
Inductive tsemax: assertS -> stmt -> assertS -> Type :=
...
- move => u0.  have h0 := semax_assign (u andS u0) x a. 
  ...
  * clear h0. move => tr0 [st0 [h0 h1]]. foo h1. foo H1.
  ...
  have := semax_conseq_R h1 h0. by apply. 
- move => u0.  
  have h0 := semax_conseq_R (@Append_assoc_R _ _ _) (IHX1 u0) => {IHX1}.
  have h: u2 ->> (u2 andS u2); first done. 
  have h1 := semax_seq h0 (semax_conseq_L h (IHX2 u2)) => {h h0 IHX2}.  
...
- move => u0. 
  ...
  (semax_ifthenelse (semax_conseq_L h (IHX1 u0)) 
  (semax_conseq_L h0 (IHX2 u0))) => {h h0 IHX1 IHX2}. apply. 
  done. move => tr0 [tr1 [h0 h1]]. move: h0 => [st0 [h0 h2]]. foo h2. 
  foo H1. foo h1. exists (Tnil st0). split. move: h0 => [_ h0]. 
  by have := mk_singleton_nil h0 => {h0}; apply. foo X. clear h0. 
  move: X0 => [tr0 [h0 h1]]. move: h0 => [st0 [h0 h2]]. foo h2. foo h1. 
  clear h0. apply follows_nil => //. move: X => [tr0 [h0 h1]].
 ...
 * move => n. have := X n. by apply.
  have h0 := semax_ex h => {X h}. 
  ...
  * clear h2. move => tr0 [x h0]. move: h0 => [tr1 [h0 h1]].
    move: h0 => [st0 [h2 h0]]. foo h0. foo h1. move: X => [tr1 [h0 h1]]. 
  ...
  * clear h0. move => tr0 [tr1 [h0 h1]]. move: h0 => [st0 [[h0 h] h2]]. 
    foo h2. foo H1. foo h1. foo X. exists (Tnil (hd tr')). split. exists (hd tr'). 
    split; [done | apply bisim_reflexive]. apply follows_nil => //. 
    move: X0 => [tr0 [h1 h2]]. exists (Tcons (hd tr') tr'). split; first last. 
    ...
    move: X => [tr1 [h0 h1]]. move: h0 => [n [tr2 [h0 h2]]]. 
    move: h0 => [st0 [h0 h3]]. foo h3. foo h2. 
    move h0: (t (hd tr1)) => n. rewrite h0 in X. clear h0.  clear h tr'. 
    ...
    - move => n tr0 tr1 tr2 h h1 h2 h3.
      ...
      - move => tr0 tr1 tr2 h0 h2 h3. foo h3. foo h0. 
        ...
        have := IHh1 _ _ _ X0 X1 X. by apply. 
    - move => n tr0 tr1 tr2 h h0 h1 h2. move: h0 => [tr3 [h0 h3]]. 
      move: tr3 h0 tr0 tr1 tr2 h3 h1 h2. induction 1. 
      - move => tr0 tr1 tr2 h0 h1 h2. foo h0. move: X => [st0 [h0 h3]]. 
        foo h3. foo h1. move: X => [st0 [h1 h3]]. foo h3. foo H1. simpl in h0. 
        foo h2. foo X. apply finite_delay. foo X0. apply finite_nil. 
        move: X => [tr0 [h2 h3]]. move: h2 =>[n0 [tr1 [h2 h4]]].
...
- clear X. move => u0. 
  ...
  * clear h0. move => st0 [h0 h1]. by split; [ have := a _ h0; apply | done]. 
  * clear h0. move => tr0 [tr1 [h0 h1]]. move: h0 => [st0 [h0 h2]]. 
    ...
    clear h0. move: X => [tr1 [h0 h1]]. exists tr1. split; first done. 
